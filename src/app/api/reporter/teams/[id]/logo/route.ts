...
// Takım logosu yükleme — multipart'ı backend'e aynen aktarır (JSON BFF
// yardımcıları multipart taşıyamadığı için özel aktarım).
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const r = await fetch(
      `${BACKEND}/api/v1/reporter/teams/${encodeURIComponent(id)}/logo`,
