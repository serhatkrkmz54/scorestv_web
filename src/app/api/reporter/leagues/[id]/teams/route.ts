...
// Atanmış ligin takımları — listele / ekle.
export async function GET(_req: NextRequest, ctx: Ctx) {
  const { id } = await ctx.params;
  const r = await authorizedBackendJson<unknown>(
    `/api/v1/reporter/leagues/${encodeURIComponent(id)}/teams`,
  );
  if (r.unauthorized) {
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  if (!r.ok || !r.body) {
    return NextResponse.json(r.body ?? { message: "Eklenemedi." }, { status: r.status });
  }
