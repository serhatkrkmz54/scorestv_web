...
// /voleybol/takim ile ayni icerik — Ingilizce URL alternatifi.
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const sp = await searchParams;
  const { data } = await fetchVolleyballTeamDetailServer(slug, "en", sp.season ?? null);
  ...
  const title = `${name} Volleyball | Scores TV`;
  const description = `${name} fixtures, results, season stats and standings — volleyball.`;
  const canonical = `${SITE}/volleyball/team/${data.slug || slug}`;
  return {
    title,
    description,
...
export default async function Page({ params, searchParams }: PageProps) {
  const { slug } = await params;
  const sp = await searchParams;
  const { data: initial, status } = await fetchVolleyballTeamDetailServer(slug, "en", sp.season ?? null);
  if (!initial) {
    if (status === 404) notFound();
    // Backend down / 5xx / zaman asimi: gercek hata firlat (error.tsx, HTTP 500).
    backendUnavailable();
  }
  const seo = await fetchVolleyballTeamSeoServer(slug, "en", sp.season ?? null);
  return (
    <>
      {seo?.jsonLd ? (
      ...
      <div className="layout">
        ...
        <div className="team-detail-main">
          ...
          <VolleyballTeamDetailScreen initial={initial} lang="en" />
        </div>
        <aside className="rail-right">
          <VolleyballTeamSideInfo detail={initial} lang="en" />
        </aside>
      </div>
    </>
  );
