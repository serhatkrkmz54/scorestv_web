...
import { volleyballLeaguePath, volleyballTeamPath } from "@/lib/routes";
...
/**
 * Voleybol sol rayi — basketbol {@link BasketballLeftRail} esi. Populer
 * ligler/takimlar backend config'inden gelir (scorestv.volleyball.serving.*).
 * BFF: /api/volleyball/leagues/popular + .../teams/popular. Liste backend'de
 * bossa ilgili bolum hic gorunmez.
 */
export function VolleyballLeftRail() {
  const { lang } = useLang();
  const t = HOME_STR[lang];
  const [leagues, setLeagues] = useState<PopularLeague[]>([]);
  const [teams, setTeams] = useState<PopularTeam[]>([]);

  // Dil degisince state'leri sifirla — hook yeni dil parametresiyle yeniden ceker.
  useEffect(() => {
  ...
  useSilentRetry<PopularLeague[]>({
    ...
    fetcher: async (signal) => {
      const r = await fetch(`/api/volleyball/leagues/popular?lang=${lang}`, {
        cache: "no-store",
        signal,
      });
      if (!r.ok) return null;
  ...
  useSilentRetry<PopularTeam[]>({
    ...
    fetcher: async (signal) => {
      const r = await fetch(`/api/volleyball/teams/popular?lang=${lang}`, {
        cache: "no-store",
        signal,
      });
      if (!r.ok) return null;
      ...
      {leagues.length > 0 && (
        <div className="rl-section">
          ...
          {leagues.map((l) => (
            <Link
              key={l.id}
              href={volleyballLeaguePath(lang, l.slug)}
              className="rl-item"
            >
      ...
      {teams.length > 0 && (
        <div className="rl-section">
          ...
          {teams.map((tm) => (
            <Link
              key={tm.id}
              href={volleyballTeamPath(lang, tm.slug)}
              className="rl-item"
            >
