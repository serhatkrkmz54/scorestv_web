...
function buildDescColorMap(groups: MatchStandingsGroup[]): Map<string, string> {
  ...
  for (const g of groups) {
    ...
    for (const r of sorted) {
      ...
      if (known) { map.set(d, known); continue; }
      while (idx < DYNAMIC_PALETTE.length && Array.from(map.values()).includes(DYNAMIC_PALETTE[idx])) idx++;
      if (idx >= DYNAMIC_PALETTE.length) idx = 0;
      map.set(d, DYNAMIC_PALETTE[idx]);
      idx++;
    }
  }
  return map;
