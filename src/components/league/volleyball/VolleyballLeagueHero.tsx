...
// Voleybol lig hero — basketbol LeagueHero esi. Sezonlar duz string listesi
// (availableSeasons); "aktif" isareti currentSeason karsilastirmasiyla.
export function VolleyballLeagueHero({
  detail,
  selectedSeason,
  onSeasonChange,
  lang,
    ...
    <section className="match-hero league-hero">
      ...
      <div className="match-hero-content league-hero-content">
        <div className="league-hero-row">
          <TeamLogo name={name} logo={detail.logo ?? null} size={72} />
          <div className="league-hero-text">
            ...
            <h1 className="league-hero-name">{name}</h1>
          </div>
              ...
              >
                {detail.availableSeasons.map((s) => (
                  <option key={s} value={s}>
                    {s}
                    {s === detail.currentSeason ? ` · ${t("Aktif", "Current")}` : ""}
                  </option>
                ))}
              </select>
            </div>
