...
import { basketballMatchPath, teamPath } from "@/lib/routes";
import { formatDate } from "@/lib/match-format";
import type {
  BasketballGameDetailResponse,
  BasketballH2hGameView,
  BasketballTeamRef,
...
export function BasketballH2HTab({ detail, lang }: Props) {
  const t = (tr: string, en: string) => (lang === "tr" ? tr : en);
  const list: BasketballH2hGameView[] = detail.headToHead ?? [];
  ...
  // Genel karne — winnerSide alanindan.
  let homeWins = 0;
  let awayWins = 0;
  ...
  for (const f of list) {
    ...
    const hHigher = (f.homeTotal ?? 0) > (f.awayTotal ?? 0);
