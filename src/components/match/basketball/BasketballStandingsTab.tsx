      ...
      <table className="standings-table">
        <thead>
          <tr>
            ...
            <th title={t("Galibiyet %", "Win %")}>%</th>
            <th title={t("Attığı Sayı", "Points For")}>SA</th>
            <th title={t("Yediği Sayı", "Points Against")}>SY</th>
            <th title={t("Sayı Farkı", "Point Diff")}>F</th>
          ...
          {rows.map((r, idx) => {
            ...
            return (
              ...
              >
                ...
                <td className="tnum">{r.pointsFor ?? 0}</td>
                <td className="tnum">{r.pointsAgainst ?? 0}</td>
                <td className="tnum">{r.pointsDifference ?? 0}</td>
...
export function BasketballStandingsTab({ detail, lang }: Props) {
  const t = (tr: string, en: string) => (lang === "tr" ? tr : en);
  const groups: BasketballStandingsGroup[] = detail.standings ?? [];
